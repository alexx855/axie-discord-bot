  ...
  try {
    ...
    if (listing.order.currentPrice !== prevFloorPrice) {
      ...
      if (floorPrice.gt(pFloorPrice)) {
        console.log('\x1b[102m%s\x1b[0m', `Axies floor price increased ${ethers.utils.formatEther(floorPrice.sub(pFloorPrice))}`)
      } else {
        console.log('\x1b[101m%s\x1b[0m', `Axies floor price dumped ${ethers.utils.formatEther(pFloorPrice.sub(floorPrice))}`)
      }
      ...
      if (priceChanged.gt(1)) {
        ...
        void DiscordRequest(`/channels/${process.env.BOT_CHANNEL_ID}/messages`, {
          ...
          body:
          {
            title: `Axie floor price changed: ${ethers.utils.formatEther(floorPrice)}`,
